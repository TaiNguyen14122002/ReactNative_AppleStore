...
const HomeScreen = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('http://172.16.10.10:8000/product');
  ...
  const fetchAddresses = async () => {
    try {
      const response = await axios.get(
        `http://172.16.10.10:8000/addresses/${userId}`,
      );
        ...
        <ScrollView>
          ...
          <View
            ...
            <Pressable
              ...
              {search
                ? products
                  .filter(item => item.title.toLowerCase().includes(search.toLowerCase())).length === 0 ? (
                  <Text style={{ padding: 20, fontSize: 16, color: 'gray' }}>
